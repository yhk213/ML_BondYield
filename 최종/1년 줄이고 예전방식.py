# -*- coding: utf-8 -*-
"""
Created on Fri Jan 22 00:48:37 2021
...
Xexog = fwd_rate.iloc[:-24,:]   # feature(forward-rate) : 1971.8 ~ 2018.12    size: 569*10
excess_return = xr_rate.iloc[12:-12,:]  # y_value : 1972.8~ 2019.12   size: 569*10   0-th column : data none
...
y_forecast_pls=[]  
for i in range(208,Xexog.shape[0]-1): 
    pls = PLSRegression(n_components = num_pls)
    
    
    pls.fit(Xexog.iloc[:i,:], excess_return.iloc[:i,maturity_n-1])
    y_forecast_pls.append(pls.predict(Xexog.iloc[i+1,:].values.reshape(1,-1)))
...
# Note that 1971.08~1988.12 : 209
for i in range(208,Xexog_X.shape[0]-1):       # traing using ~88.12 > using 89.01 data and forecast 90.01 
    ...
    x_new_macro_scaled = Xexog_X_scaler.transform(Xexog_X.iloc[i+1,:].values.reshape(1,-1))  #start 1989.01  
...
y_forecast_pca_macro = np.array(y_forecas_pca_macro).reshape(-1)

print('R2oos with pca, with macro' + ' (#pca compo) ' +str(num_pca_macro) + ' (maturity) ' +str(maturity_n_macro)+' : ' +str(R2OOS(y_true_macro, y_forecast_macro_pca)))
"""